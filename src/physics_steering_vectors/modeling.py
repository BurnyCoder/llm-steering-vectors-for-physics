...
Local Function:
- Loads a Qwen text model, tokenizer, and steering hook config.
...
from transformers import AutoModelForCausalLM, AutoTokenizer  # Source: Qwen2.5 model card. Local: documented loaders. Global: load selected checkpoint.
...
def load_qwen_bundle(config: ExperimentConfig) -> ModelBundle:
    """Load the configured Qwen text checkpoint.
    ...
    Local Function:
    - Instantiates model, tokenizer, and layer config.
    ...
    return ModelBundle(  # Local: package runtime objects. Global: standard phase output.
        ...
        processor=None,  # Local: text-only model has no processor. Global: preserve bundle shape for callers.
...
def model_device(model: torch.nn.Module) -> torch.device:
    ...
    Sources Used:
    - Transformers generation examples: https://huggingface.co/docs/transformers/model_doc/qwen2
