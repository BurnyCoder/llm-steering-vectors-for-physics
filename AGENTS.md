...
Do not commit downloaded models, datasets, generated caches, generated steering-vector `.pt` files, or virtual environments. Keep local secrets and Hugging Face credentials outside the repository. Treat `max_test_examples`, `train_generations_per_question`, training sampling settings, model IDs, layer sweeps, multipliers, and `steering_vector_dir` as protocol-affecting settings and document any changes in PRs.
